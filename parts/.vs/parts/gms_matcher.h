...
public:
	...
	void GetInlierMask(vector<bool> &vbInliers, bool WithScale = false, bool WithRotation = false);
...
void gms_matcher::GetInlierMask(vector<bool> &vbInliers, bool WithScale, bool WithRotation) {
...
void gms_matcher::AssignMatchPairs(int GridType) {
	...
	for (size_t i = 0; i < mNumberMatches; i++)
	{
		...
		//GetGridIndexLeft函数是对归一化匹配对乘以它的网格大小，并返回x + y * mGridSizeLeft.width;
		int rgidx = -1;

		if (GridType == 1)
		{
			rgidx = mvMatchPairs[i].second = GetGridIndexRight(rp);
			//GetGridIndexRight函数是对归一化点乘以网格大小，并返回 x + y * mGridSizeRight.width;
		}
		else
		{
		...
		//cout << "mMotionStatistics" << mMotionStatistics.at<int>(lgidx, rgidx) << endl;
		//cout << "mNumberPointsInPerCellLeft" << mNumberPointsInPerCellLeft[lgidx] << endl;
		//cout << "...." << endl;
	}
...
void gms_matcher::VerifyCellPairs(int RotationType) {
	...
	for (int i = 0; i < mGridNumberLeft; i++)
	{
		...
		thresh = THRESH_FACTOR * sqrt(thresh / numpair);//THRESH_FACTOR就是论文中的alph
...
int gms_matcher::run(int RotationType) {
	...
	mvbInlierMask.assign(mNumberMatches, false);//mvbInlierMask大小等于mNumberMatches,值全为false
	...
	for (int GridType = 1; GridType <= 4; GridType++) 
	{
		...
		AssignMatchPairs(GridType);//计算对应网格有多少匹配对，特征点
